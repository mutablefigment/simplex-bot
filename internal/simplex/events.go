...
// File is an inbound attachment offered on a received chat item. It carries
// only the metadata needed to fetch it — the bot chooses the destination and
// calls ReceiveFile to pull the bytes down to it.
type File struct {
	ID   int64
	Name string
	Size int64
